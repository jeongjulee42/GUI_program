# 버그 수정
# 저장경로 폴더 선택 이후 한번 더 찾고 취소를 누르면 경로가 사라진다.-> def browse_dest_path 에서 if folder_selected is None
# 즉 사용자가 취소를 누를때 그냥 return 하기 때문
# 수정

# 저장 경로를 c:/를 직접적으로 한 경우 windows 10에서 접근 권한이 없다고 뜬다.
# 관리자 권한이 필요하다.
# 또한 존재하지 않는 곳에 넣는 경우 에러가 떠야하지만 프로그램상에서는 메세지가 없다.
# def merge 함수에서 해결
...
def browse_dest_path():
    ...
    if folder_selected == '':  # 빈 문자열로 수정한다.
        return
...
def merge_image():
    try:
        ...
        for idx, img in enumerate(images):
            ...
            # 여기 수정, height값에다 사용자 지정 간격을 더한다.
