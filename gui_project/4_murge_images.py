...
def add_file():
    # 파일 다이얼로그  - from tkinter import filedialog 사용. __all__ 때문에 따로 import 한다.
    ...
    # initialdir - 최초의 경로
    # initialdir=r"/Users/jeongju/Desktop" 같이 r을 추가하면 있는 그대로 쓰여진다.
    # 사용자가 선택한 파일 목록을 출력
    for file in files:
...
def merge_image():
    ...
    # size라는 정보가 있음 size[0] - width , size[1] - height
    # widths = [x.size[0] for x in images]
    # heights = [x.size[1] for x in images]
    widths, heights = zip(*(x.size for x in images))
    # 큰 스케치북을 만들어야 하는데 이때 width는 제일 큰것을 기준, height는 이미지들을 모두 더해 길이를 계산해야 한다.
    max_width, total_height = max(widths), sum(heights)

    # 스케치북 준비
    result_img = Image.new(
        "RGB", (max_width, total_height), (255, 255, 255))  # 배경 흰색
    y_offset = 0  # 각 이미지를 붙일 때 붙이는 위치.
    # for img in images:
    #     result_img.paste(img, (0, y_offset))
    #     y_offset += img.size[1]  # height값만큼 더해준다
    # progressbar와의 연동을 위해 추가
